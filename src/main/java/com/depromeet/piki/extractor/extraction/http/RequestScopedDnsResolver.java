 ...
 *
 * <p>{@link InternalHostGuard} 가 host→IP 를 한 번 조회해 사설/내부 IP 를 차단한 뒤, 실제 연결(HttpClient5 의
 ...
 *
 * <p>fetch 한 번이 끝나면 clear() 로 비워 다음 요청과 격리한다. 동기 호출(HttpComponents)이라 가드와 연결이 같은
...
public class RequestScopedDnsResolver {
    ...
    /**
     * host→IP 실제 조회 위임. 테스트가 가짜 IP 를 주입해 SSRF 가드/캐시 로직을 네트워크 없이 검증하도록 교체점을
     * 연다. InetAddress.getAllByName 이 checked UnknownHostException 을 던지므로 전용 함수형 인터페이스로 그
     * 시그니처를 그대로 표현한다.
     */
    @FunctionalInterface
