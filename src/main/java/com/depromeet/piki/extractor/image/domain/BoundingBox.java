...
/**
 * Gemini 가 반환하는 상품 영역 박스. 좌표는 0~1000 normalized(Gemini 표준 {@code [ymin, xmin, ymax, xmax]})이고,
 * 픽셀 환산은 {@code ImageCropper} 가 원본 이미지 크기에 맞춰 수행한다.
 */
public record BoundingBox(
    ...
    /**
     * 불변식 — 정상 흐름은 {@code ofNormalizedOrNull} 이 먼저 거른다. 직접 생성으로 비정상 좌표가 들어오면
     * 크롭이 조용히 어긋나므로 생성 시점에 막는다.
     */
    public BoundingBox {
    ...
    /**
     * 좌표가 하나라도 빠지거나 비정상이면 null — 크롭을 건너뛰는 신호다
     * ({@code CurrencyCode.normalizeOrNull} 과 같은 "OrNull" 규약).
     */
