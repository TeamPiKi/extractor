...
/**
 * Gemini JSON Schema 파서는 {@code "properties": null} 같은 잉여 null 필드를 스키마 위반으로 취급하므로,
 * 직렬화 단계에서 null 필드를 전부 생략한다({@code GeminiExtractionRequest} 와 동일 정책).
 *
 * <p>주의: 이미지 경로의 wire 는 link 경로({@code GeminiExtractionRequest})와 다르다 — 필드명은
 * {@code responseJsonSchema} 가 아니라 {@code responseSchema}, 스키마 type 은 소문자 문자열이 아니라
 * {@code SchemaType} enum(직렬화 시 "OBJECT"/"STRING"/"INTEGER" 대문자), 그리고 이미지 part 를 담는
 * sealed {@code Part}(Text/Image) 가 있다.
 */
...
public record GeminiImageRequest(
    ...
    /**
     * 텍스트 파트와 이미지 파트가 한 리스트에 섞여 온다. 이 요청은 직렬화만 되고 역직렬화되지 않으므로
     * 타입 판별자(discriminator)가 붙지 않는다 — Jackson 은 런타임 타입의 컴포넌트만 찍어
     * {@code {"text":...}} / {@code {"inlineData":...}} 로 나간다.
     */
    ...
    /**
     * thinkingLevel 을 명시하지 않으면 dynamic thinking 이 붙어, 이미지에서 값을 직접 읽는 단순한 작업에도
     * 응답이 간헐적으로 ~20s 까지 튄다(read-timeout 초과 → 호출 실패). 깊은 추론이 불필요하므로 minimal 로
     * 고정해 응답 시간을 안정화한다.
     */
    ...
    /** LinkedHashMap 으로 스키마 property 의 선언 순서를 고정한다. */
