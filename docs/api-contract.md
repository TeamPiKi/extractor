...
| 요청·응답의 와이어 모양 (빌드 시점에 클래스 생성, 파일럿: link 경로) | `contracts/extraction.proto` |
...
카탈로그와 proto 는 로컬에서 infra 의 `install.sh` 가, CI 에서는 `ci.yml`·`deploy.yml` 의 checkout 스텝이 `shared-infra/contracts/` 에 놓는다. proto 는 `generateProto` 가 읽어 `com.depromeet.piki.contracts.extraction.v1` 패키지의 클래스를 만들고, `ExtractionErrorCodeCatalogTest` 가 그 enum 도 카탈로그와 대조한다. 와이어는 JSON 그대로다(`ContractMessageConverterConfig`).
